	...
	*/
	...
	protected void conectar(String servidor, int porta) throws Exception {
		...
		try {
			...
			SkStatus.logInfo("<strong>" + mContext.getString(+R.string.state_connecting) + "</strong>");
	...
	protected void autenticar(String usuario, String senha, String keyPath) throws IOException {
		...
		try {
			if (mConnection.isAuthMethodAvailable(usuario,
				...
				SkStatus.logInfo("<strong>Autenticando com senha");
		...
		if (!mConnection.isAuthenticationComplete()) {
			SkStatus.logInfo("<strong></font><font color=\"yellow\">Senha incorreta \uD83D\uDD11</strong>");
	...
	public void connectionLost(Throwable reason)
	{
		...
		SkStatus.logError("<strong><font color=\"red\">" + mContext.getString(R.string.log_conection_lost) + "</strong>");
	...
	public void reconnectSSH() {
		...
		for (int i = 0; i < RECONNECT_TRIES; i++) {
			...
			if (!TunnelUtils.isNetworkOnline(mContext)) {
				...
				SkStatus.logInfo("<strong><font color=\"#00ced1\">" + mContext.getString(R.string.state_nonetwork) + "</strong>");
			}
			else {
				sleepTime = 3;
				mStarting = true;
